...
public Device update(int id, Device dev) throws Exception{
	Optional<Device> deviceResult = dr.findById(id);
	if(deviceResult.isPresent()) {
		Device device = deviceResult.get();
		device.setType(dev.getType());
		device.setState(dev.getState());
		dr.save(device);
		return device;
	} else {
		throw new Exception("Modifica non riuscita");
	}
}
