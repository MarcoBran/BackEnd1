...
public User save(User u) {
	...
	if(check == true) {
		System.out.println("Username già utilizzato! Sceglierne un altro.");
		System.exit(0);
	}
	return ur.save(u);
}
...
public User giveDevice(int userId,  int deviceId) throws Exception {
	...
	Optional<Device> deviceResult = dr.findById(deviceId);
	if(userResult.isPresent()&&deviceResult.isPresent()) {
		...
		throw new Exception("Valori non aggionati");
	}
}
